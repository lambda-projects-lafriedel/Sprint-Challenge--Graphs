...
def populate_graph_with_exits(room):
    # add the roomid as a key in the graph with value of empty dict
    graph[room.id] = dict()
    # get all the room's exits
    exits = room.getExits()
    # for each direction, add the direction as a key to the room's dict and a value of ?
    for direction in exits:
        graph[room.id][direction] = '?'

# gets opposite direction traveled
...
def bfs(room):
    ...
    for move in reversePath:
        ...
        if '?' in graph[player.currentRoom.id].values():
            ...
            reversePath.clear()
            print("VISITED AT END OF BFS", len(visited_rooms))
...
def dfs(room, directions):
    ...
    # get first unexplored direction
    direction = directions.pop(0)

    # travel in that direction
    player.travel(direction) # player.currentRoom is now changed
    # add that direction to the traversalPath
    traversalPath.append(direction)
    # get opposite direction of one just traveled
    opposite = get_opposite(direction)
    # add opposite direction to reverse path
    reversePath.append(opposite)
    ...
    if player.currentRoom.id not in graph:
        ...
        graph[curr_room][direction] = player.currentRoom.id
        # save the opposite direction traveled in the graph so we know the rooms are connected
        graph[player.currentRoom.id][opposite] = curr_room
    # else
    else:
        # just save the room as a value for the previous room's direction key
        graph[curr_room][direction] = player.currentRoom.id
    #print("TRAV PATH AT END OF DFS", player.currentRoom.id, traversalPath)
    print("VISITED AT END OF DFS", len(visited_rooms))
    print("GRAPH AT END OF DFS", graph)
...
while len(graph) < len(roomGraph):
    ...
    unexplored_exits = []
    # for each exit from the current room
    for direction, room in graph[player.currentRoom.id].items():
        # if the rooms's exits contain a '?' as their value, add them to unexplored_exits
        if room == '?':
            unexplored_exits.append(direction)

    # if there's 1 or more unexplored exit, explore!
    if len(unexplored_exits) > 0:
        # call dfs and explore
        dfs(player.currentRoom, unexplored_exits)
    ...
    #print("TRAV PATH AT END OX EXPLORE", traversalPath)
    print("VISITED AT END OF EXPLORE", len(visited_rooms))
    print("GRAPH AT END OF EXPLORE", graph)
